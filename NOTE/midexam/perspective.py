...
def main():
    ...
    # b = [404, 275, 3376, 3520, 1077, 1636, 1198, 1674]  #3.jpg
    # b = [66, 54, 1092, 1100, 174, 904, 61, 948]  #4.jpg
    # x1,x2,x3,x4,y1,y2,y3,y4
    b = [int(b) for b in sys.argv[2].split(',')]
    A = np.zeros((max_matrix, max_matrix), dtype=np.float64)

    # 取得輸出圖片out_x、out_y產生output_img
    out_x = abs(b[3] - b[0])
    out_y = abs(b[4] - b[5])
    output_img = np.zeros((out_y, out_x, 3), dtype=input_img.dtype)  # 4.jpg

    rows = output_img.shape[0]
    cols = output_img.shape[1]
    corner = Corner(rows, cols).get_corner()

    init_A(A, corner, max_matrix)   # 組裝出Ax=b相關必要參數
    b = gauss_jordan(A, b)          # 求出八個參數

    [print(f'b[{i}]={b[i]}') for i in range(max_matrix)]    #印出b的8個參數

    invert_mapping(b, input_img, output_img)    # 得出輸出圖
...
# 傳入八個參數及原圖及輸出圖，代公代並採用invert mapping進行最後輸出
